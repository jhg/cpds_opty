...
% Commit transaction
do_transactions(Name, Entries, Updates, Reads, Server, Handler, Total, Ok, 0, 0) ->
    %io:format("~w: Commit: TOTAL ~w, OK ~w~n", [Name, Total, Ok]),
    %timer:sleep(Name*10),
    Ref = make_ref(),
    Handler ! {commit, Ref},
...
% Reads and Writes
do_transactions(Name, Entries, Updates, Reads, Server, Handler, Total, Ok, Nwrites, Nreads) ->
    %io:format("~w: R/W: TOTAL ~w, OK ~w, N ~w~n", [Name, Total, Ok, N]),
    N = random:uniform(),
    if
        N >= 0.5 ->
            do_read(Name, Entries, Updates, Reads, Server, Handler, Total, Ok, Nwrites, Nreads);
        true ->
            do_write(Name, Entries, Updates, Reads, Server, Handler, Total, Ok, Nwrites, Nreads)
    end.
...
do_read(Name, Entries, Updates, Reads, Server, Handler, Total, Ok, Nwrites, Nreads) ->
    if
        ...
        true ->
            ...
            Handler ! {read, Ref, Num},
            do_transactions(Name, Entries, Updates, Reads, Server, Handler, Total, Ok, Nwrites, Nreads-1)
    end.
...
do_write(Name, Entries, Updates, Reads, Server, Handler, Total, Ok, Nwrites, Nreads) ->
    if
        ...
        true ->
            ...
            Value = receiveValue(Ref),
            Handler ! {write, Num, Value+1},
            do_transactions(Name, Entries, Updates, Reads, Server, Handler, Total, Ok, Nwrites-1, Nreads)
    end.

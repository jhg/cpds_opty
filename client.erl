...
start(Name, Entries, Updates, Server, NSubset) ->
    spawn(fun() -> open(Name, Entries, Updates, Server, NSubset, 0, 0) end).

open(Name, Entries, Updates, Server, NSubset, Total, Ok) ->
    {A1,A2,A3} = now(),
    random:seed(A1, A2, A3),
    Server ! {open, self()},
    receive
        ...
        {transaction, Validator, Store} ->
            ...
            do_transactions(Name, Entries, Updates, Server, Handler, NSubset, 
                            Total, Ok, Updates, Subset)
    end.
...
% Commit transaction
do_transactions(Name, Entries, Updates, Server, Handler, NSubset, Total, Ok, 0, _) ->
    %io:format("~w: Commit: TOTAL ~w, OK ~w~n", [Name, Total, Ok]),
    %timer:sleep(Name*10),
    Ref = make_ref(),
    Handler ! {commit, Ref},
    ...
    if
        Result == ok ->
            open(Name, Entries, Updates, Server, NSubset, Total+1, Ok+1);
        true ->
            open(Name, Entries, Updates, Server, NSubset, Total+1, Ok)
    end;

% Reads and Writes
do_transactions(Name, Entries, Updates, Server, Handler, NSubset, Total, Ok, N, Subset) ->
    %io:format("~w: R/W: TOTAL ~w, OK ~w, N ~w~n", [Name, Total, Ok, N]),
    Ref = make_ref(),
    ...
    Value = receiveValue(Ref),
    Handler ! {write, Num, Value+1},
    do_transactions(Name, Entries, Updates, Server, Handler, NSubset, Total, Ok, N-1, Subset).
